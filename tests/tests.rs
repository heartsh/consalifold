...
fn test_consalifold() {
  ...
  let basepairs = consalifold::<u8>(&basepair_probs_mix, &align, hyperparam);
  let mut basepairs_hashed = SparsePoss::<u8>::default();
  for x in &basepairs {
    match basepairs_hashed.get(&x.0) {
      Some(_) => {
        panic!();
      }
      None => {
        basepairs_hashed.insert(x.0);
      }
    }
    match basepairs_hashed.get(&x.1) {
      Some(_) => {
        panic!();
      }
      None => {
        basepairs_hashed.insert(x.1);
      }
    }
  }
}
